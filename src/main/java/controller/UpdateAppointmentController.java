...
public class UpdateAppointmentController implements Initializable {
    ...
    /**
     * gets and sets data from appointment records
     * @param selectedItem
     * @throws SQLException
     */
    public void setData(Appointments selectedItem) throws SQLException {
        Appointment_ID.setText(String.valueOf(selectedItem.getAppointmentId()));
        Title.setText(String.valueOf(selectedItem.getTitle()));
        Type.setText(String.valueOf(selectedItem.getType()));
        Description.setText(String.valueOf(selectedItem.getDescription()));
        Location.setText(String.valueOf(selectedItem.getLocation()));
        Start_date.setValue(LocalDate.parse((selectedItem.getStart().substring(0,10))));
        End_date.setValue(LocalDate.parse(selectedItem.getEnd().substring(0,10)));
        Contact.setValue(String.valueOf(selectedItem.getContactId()));
        User_ID.setValue(String.valueOf(selectedItem.getUserId()));
        Start_hour.setValue(selectedItem.getStart().substring(12,14));
        End_hour.setValue(selectedItem.getEnd().substring(12,14));
        Contact.setValue(selectedItem.getContactId() + "  " + ContactDAO.getContactName(selectedItem.getContactId()));
        Customer_ID.setValue(selectedItem.getCustomerId());
        Start_minute.setValue(selectedItem.getStart().substring(15,17));
        End_minute.setValue(selectedItem.getEnd().substring(15,17));
        SAP.setValue(selectedItem.getStart().substring(18,20));
        EAP.setValue(selectedItem.getEnd().substring(18,20));


    }

    /**
     * updates appointment in appointment table
     * @param actionEvent
     * @throws SQLException
     * @throws IOException
     */
    public void Update_press(ActionEvent actionEvent) throws SQLException, IOException {
        ...
        int userId = Integer.parseInt(User_ID.getValue().toString());
        int customerId = Integer.parseInt(Customer_ID.getValue().toString());
        ...
        try {
            ...
            for(int i =0; i < cusApp.size();i++){
                if(appointmentId != cusApp.get(i).getAppointmentId()){
        ...
        AppointmentDAO.updateAppointment(title, description, location, type, Timestamp.valueOf(SDT), Timestamp.valueOf(EDT), userName,
    ...
    /**
     * goes back to the appointments scene
     * @param actionEvent
     * @throws IOException
     */
